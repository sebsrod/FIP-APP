  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col bg-zinc-950">
      ...
      <header className="flex shrink-0 items-center gap-2 border-b border-white/10 px-3 py-3">
        ...
        <div className="flex flex-1 items-center gap-2 rounded-full border border-white/10 bg-black/40 px-3.5 py-2.5 focus-within:border-rose-500/60">
          ...
          <input
            ...
            autoCorrect="off"
            spellCheck={false}
            className="w-full bg-transparent text-base text-zinc-50 placeholder:text-zinc-500 focus:outline-none"
          />
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto">
        {status === 'idle' ? (
          ...
          <p className="px-5 py-6 text-sm text-zinc-500">Searching…</p>
        ) : status === 'error' ? (
          <p className="px-5 py-6 text-sm text-rose-400">Something went wrong. Try again.</p>
        ) : results.length === 0 ? (
              ...
              <li key={u.id}>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-medium text-zinc-100">{u.display_name}</div>
                    <div className="truncate text-xs text-zinc-500">@{u.username}</div>
                  </div>
                  <div className="shrink-0 text-right text-[11px] text-zinc-500">
                    <span className="tabular-nums text-zinc-300">{formatCount(u.followers_count)}</span> followers
                  </div>
                </button>
              </li>
            ))}
