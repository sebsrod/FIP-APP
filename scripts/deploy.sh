...
if [ -n "$URL" ]; then
  ...
  echo "    GET  /api/auth/me           -> $ME    (expect 200, guest session)"
  echo "    POST /api/auth/login (demo) -> $LOGIN (expect 200)"
  if [ "$ME" = "200" ] && [ "$LOGIN" = "200" ]; then
    printf '\n\033[1;32m✅ Deployed & verified.\033[0m Open %s — browse as a guest, or log in with demo / demo1234\n' "$URL"
  else
